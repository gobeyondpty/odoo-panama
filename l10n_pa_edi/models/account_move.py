...
# Full ISO 3166-1 alpha-2 → alpha-3 table. The DGI XML expects alpha-3
# (`cPaisRec`/`dPaisExt`); upstream `res.country` only stores alpha-2.
# Generated from the Debian `iso-codes` dataset (iso_3166-1.json,
# 249 entries); falls back to the alpha-2 code padded if not found.
_COUNTRY_ALPHA3 = {
    ...
    # Non-ISO codes shipped by Odoo's res.country: Kosovo uses the
    # user-assigned XKX (EU/World Bank convention); Northern Ireland
    # (XI, a UK trade-protocol code) is reported as GBR territory.
    'XK': 'XKX', 'XI': 'GBR',
}
